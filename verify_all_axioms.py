...
for name, cid, attr_name, aid, target_name, tid in axioms_to_verify:
    ...
    # 1. Validate codes
    v_c = client.validate(cid)
    v_a = client.validate(aid)
    v_t = client.validate(tid)
    print(f"  Codes valid? Concept: {v_c}, Attr: {v_a}, Target: {v_t}")
    
    # 2. Verify relationship
    # ECL: ConceptID : AttributeID = TargetID
    ecl = f"{cid} : {aid} = {tid}"
    results = client.get_ecl(ecl)
    if results:
        print(f"  Relationship Verified: YES")
    else:
        print(f"  Relationship Verified: NO (Check definition in browser)")
